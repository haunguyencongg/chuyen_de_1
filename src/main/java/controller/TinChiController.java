...
public class TinChiController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(add > 0) {
			response.sendRedirect(request.getContextPath()+"/tin-chi?msg=OK");
			return;
		}else {
			response.sendRedirect(request.getContextPath()+"/tin-chi?msg=ERROR");
			return;
		}
