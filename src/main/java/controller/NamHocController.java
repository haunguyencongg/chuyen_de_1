...
@WebServlet(urlPatterns = {"/nam-hoc"})
public class NamHocController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(add > 0) {
			response.sendRedirect(request.getContextPath()+"/nam-hoc?msg=OK");
			return;
		}else {
			response.sendRedirect(request.getContextPath()+"/nam-hoc?msg=ERROR");
			return;
		}
